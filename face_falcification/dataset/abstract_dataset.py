...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def init_data_aug_method(self):
        print(self.config['with_landmark'])
        ...
        trans = A.Compose([
            ...
            # ], p = 0 if self.config['with_landmark'] else 1), #sahar
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        try:
            #print(os.path.join(self.config['dataset_json_folder'], dataset_name + '.json'))
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                if self.frame_num < total_frames:
                    ...
                    if self.video_level:
                        ...
                        root = self.config.get('dataset_root_rgb', '')  
                        # frame_paths = [frame_paths[i] for i in range(0, total_frames, step)][:self.frame_num]
                        frame_paths = [os.path.join(root, frame_paths[i]) for i in range(0, total_frames, step)][:self.frame_num]
                        # print(frame_paths)
                        # quit()
