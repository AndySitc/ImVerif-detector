...
def train(args, model, train_loader, test_loader, device, logger, optimizers):
    ...
    for epoch in trange(args.n_epochs, desc='Epoch'):
        ...
        model.train()
        print(f"Type de train_loader.dataset.dataset: {type(train_loader.dataset.dataset)}")
...
if __name__ == '__main__':
    ...
    args.n_data = len(train_loader)
    args.image_dims = train_loader.image_dims
    ...
    train_length = int(0.8 * len(train_loader))
    # test_length = int(0.2 * len(train_loader))
    test_length = len(train_loader) - train_length
