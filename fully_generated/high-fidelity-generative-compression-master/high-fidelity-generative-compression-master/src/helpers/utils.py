...
def quick_restore_model(model, filename):
    ...
    model.load_state_dict(checkpt["state_dict"], strict=False)
    return model
...
def load_model(save_path, logger, device, model_type=None, model_mode=None, current_args_d=None, prediction=True, 
    ...
    if prediction is True:
        ...
        # print(model.load_state_dict(checkpoint['compression_model_state_dict']))
        # model.load_state_dict(checkpoint['model_state_dict'])
        # print("Checkpoint keys:", checkpoint.keys())
        # print("Checkpoint parameter groups:", checkpoint['compression_optimizer_state_dict']['param_groups'])
        # print("Current optimizer parameter groups:", optimizers['amort'].state_dict()['param_groups'])

        # optimizers['amort'].load_state_dict(checkpoint['compression_optimizer_state_dict'])
        optimizers['amort'] = torch.optim.Adam(model.parameters(), lr=0.0001)  # Ajustez les hyperparamètres si besoin
...
def save_images(writer, step, real, decoded, fname):
    ...
    target_size = (192, 192) 
    real = F.interpolate(real, size=target_size, mode='bilinear', align_corners=False)
