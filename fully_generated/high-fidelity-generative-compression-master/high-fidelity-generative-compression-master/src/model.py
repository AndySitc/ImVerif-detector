...
class Model(nn.Module):
    ...
    def compression_forward(self, x):
        ...
        return intermediates, hyperinfo, y
    ...
    def discriminator_forward(self, intermediates, train_generator):
        ...
        target_size = (192, 192)  
        x_gen = F.interpolate(x_gen, size=target_size, mode='bilinear', align_corners=False)
        x_real = F.interpolate(x_real, size=target_size, mode='bilinear', align_corners=False)
        # D_in = torch.cat([x_real_resized, x_gen], dim=0)
    ...
    def distortion_loss(self, x_gen, x_real):
        ...
        # x_real_resized = F.interpolate(x_real, size=(192, 192), mode='bilinear', align_corners=False)
        target_size = (192, 192)  # Ou une taille spécifique à LPIPS si nécessaire

        x_gen = F.interpolate(x_gen, size=target_size, mode='bilinear', align_corners=False)
        x_real = F.interpolate(x_real, size=target_size, mode='bilinear', align_corners=False)
        # print(f"x_gen shape: {x_gen.shape}, x_real shape: {x_real.shape}")
        # sq_err = self.squared_difference(x_gen*200., x_real*200.) # / 255.
        sq_err = self.squared_difference(x_gen * 200., x_real * 200.)
    ...
    def compress(self, x, silent=False):
        ...
        * Encode latents via entropy model derived from (mean, scale) hyperprior output.
        ...
        if self.model_mode == ModelModes.EVALUATION and (self.training is False):
            n_hyperencoder_downsamples = self.Hyperprior.analysis_net.n_downsampling_layers
            factor = 2 ** n_hyperencoder_downsamples
            y = utils.pad_factor(y_latent, y_latent.size()[2:], factor)
        ...
        return y_latent, compression_output
    ...
    def forward(self, x, train_generator=False, return_intermediates=False, writeout=True):
        ...
        intermediates, hyperinfo, y = self.compression_forward(x)
        return intermediates, hyperinfo, y
